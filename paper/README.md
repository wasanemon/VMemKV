...
- `system_contract.md`
  - 論文執筆時の最初の参照先です。
  - 現在ブランチの `implementation/` と `implementation/TODO.md` を統合し、
    現行 prototype と final-state system を分けて整理しています。

- `evalutation/`
  - 評価計画として重要な内容です。
  - 評価計画、ワークロード整理、追加で読むべき文献などをまとめています。
  - システム仕様は `system_contract.md` を先に確認してから参照してください。

## 論文執筆の進め方

論文執筆は、まず評価計画(`evaluation_plan.md`)を固め、それを軸に本文の主張・構成・図表を組み立てます。
システムの説明は `system_contract.md` に従い、評価で示せる範囲を超えた主張は本文に入れない方針です。

## 優先度が低いもの

- `obsolete_drafts/`
  - 古い論文下書き・レビュー・作業メモのアーカイブです。
  - 現在の仕様や論文方針を表すものではありません。
  - 特に指定がない限り、無視してください
...
特に `obsolete_drafts/` は過去の検討記録としてのみ扱い、仕様判断の根拠にしないでください。
