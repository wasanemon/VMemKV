...
auto T2FlatFile::at(uint64_t payload, const std::shared_ptr<const T2Memory> &mem) const noexcept -> T2RecordView {
...
auto T2FlatFile::append(std::span<const std::byte> key, std::span<const std::byte> value) -> uint64_t {
