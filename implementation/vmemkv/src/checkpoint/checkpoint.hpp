...
// the directory's boundary keys once the whole call returns -- the full entry set is never
// materialized as one array, and neither shard_count nor the boundary keys are known until after
// every shard has already been written. So the header is a *trailer* here, not a prefix: on-disk
// layout is `shard_count` sections of (entry_count: uint64_t, entries[entry_count]), then
// `boundary_count` T1ChkKeyPrefix boundary keys, then the header, last. This lets the writer be
// purely sequential (no seek-back-and-overwrite once final counts are known) at the cost of the
// reader locating the header via `file_size - sizeof(header)` instead of offset 0 -- trivial with
// mmap. Shard sections have no fixed stride (entry counts vary), so a reader walks them
// sequentially using each section's own entry_count. Written once to a temp path, fsynced, and
// only reachable once the manifest commits to its generation.
