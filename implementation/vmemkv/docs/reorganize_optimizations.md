...
| 方向性 | 状態 | 要約 |
| --- | --- | --- |
| スレッド並列マージ | プロトタイプ実施・見送り(実機32コア・10億件でも再検証済み、結論変わらず) | 個々のワーカーは高速化するが、結合コピー+`SortedRegion`構築が未並列のまま残り、全体では効果なし |
...
### 1.1 AWS実機(r6i.8xlarge, 32コア, 10億件)での検証

上記の測定はローカル・約300万件時点のもの。「10億件まで増やしたときの内訳」を検証するため、
線形外挿予測(merge~16s, ctor~16.6s @1B)をAWS spotインスタンス(r6i.8xlarge, 256GB RAM/32vCPU,
`ap-northeast-1d`)上で実測した。スタンドアロンハーネスの`T1AppendCapacityLog2`を27
(1.34億件のappend領域)に引き上げ、batch=1億件/サイクル×10サイクルで合計10億件まで積み上げながら
各サイクルの`reorganize()`を計測。

**実測結果(最終、コーパス=10億件)**:

| 構成 | マージ | `SortedRegion`コンストラクタ | その他* | 合計 | speedup |
| --- | --- | --- | --- | --- | --- |
| sequential | 20.91s | 18.54s | 35.91s | 75.36s | - |
| parallel-4 | 21.12s | 18.57s | 35.48s | 75.17s | 1.00x |
| parallel-32 | 16.05s | 18.57s | 35.55s | 70.17s | 1.07x |

*freeze/epoch-wait/append領域収集など。

**ローカル300万件で見えていた「並列化してもほぼ速くならない」という結論は、実機32コア・
10億件規模でも変わらなかった**(4threadで1.00x、32threadで1.07x)。

新たに判明した点:

1. 「その他」はコーパスサイズではなく**batch(delta)サイズにほぼ比例するO(k)**であることが
   実測でも裏付けられた: コーパスが100M→1000M件と10倍に増えても「その他」はほぼ一定
   (33.75s→35.91s、+6%)。ただしこの絶対値自体は、put()を成功させるためappend容量を本番
   デフォルト(`T1AppendCapacityLog2=21`≈210万件)よりはるかに大きい2^27(1.34億件)へ引き上げ、
   batchを1億件/サイクルにした本ベンチマーク特有の値であり、本番相当の設定では「その他」は
   数十〜数百ミリ秒のオーダーに収まるはず。
2. **parallel-32が32倍近いスピードアップを出せない直接原因が、ワーカー単位のタイミングから
   判明した**: 本ベンチマークのキー生成が単調増加(`"key" + std::to_string(10^10 + i)`)のため、
   新規挿入分(`imm_entries`)は常に`sorted_region_`の既存キー全件より辞書順で後ろに来る。
   境界を`lower_bound`で揃える現行の等分割方式では、この場合ほぼ全ての`imm_entries`が
   最後の1パーティションに集中してしまう。例えばコーパス9億件時点のサイクルでは、通常ワーカーが
   約2800万件を約1.2秒で処理する一方、最後のワーカー(#31)だけ1.28億件(約4.6倍)を2.85秒で
   処理していた。並列マージの所要時間は最も遅いワーカーで律速されるため、32分割してもボトルネックは
   ほとんど解消されない。**単調増加キー(auto-increment ID・タイムスタンプ主キーなど)は、この
   素朴な「等分割+lower_bound」方式の実務上の弱点**として記録しておく価値がある——キー分布が
   ランダムに近ければこの偏りは起きにくいが、シーケンシャルな挿入パターンを持つワークロードでは
   現実にも起こりうる。
3. マージ・`SortedRegion`コンストラクタとも、線形性を維持したままスケールした: 100M→1000M件で
   ほぼ10倍(マージ: 1.27s→20.91s、ctor: 1.87s→18.54s)。以前の3M件からの線形外挿(merge~16s,
   ctor~16.6s @1B)とオーダーは一致、実測はやや高め(21s/18.5s、+30%程度)——外挿時に挙げていた
   TLB圧やアロケーションサイズ増大による超線形劣化の可能性が、多少なりとも効いている可能性がある。

計測環境: AWS EC2 r6i.8xlarge(spot、`ap-northeast-1d`、256GB RAM/32vCPU)、Ubuntu 24.04、
g++ 13.3.0、`-std=c++20 -O2 -DNDEBUG -pthread`。ハーネスは`prototype-parallel-reorganize-merge`
ブランチの`T1Index`/`SmallThreadPool`をそのまま流用。計測後インスタンスは削除済み。
...
- **次に試す最有力候補**: 既存の逐次マージループをbranchless(cmov)スタイルに書き換え、
  ...
  命令レベル並列を積み上げる。
- スレッド並列マージについては、AWS実機(32コア・10億件、1.1節)でも「ほぼ効果なし」という
  結論がそのまま再現された。ただし単調増加キーによるパーティション偏りという新しい弱点が判明した
  ため、将来Merge Path流の分割統治を採用する際は、単純な等分割+`lower_bound`ではなく偏りに
  強い分割方式(例: 各パーティションの見込み件数を動的に調整する)を検討する価値がある。
